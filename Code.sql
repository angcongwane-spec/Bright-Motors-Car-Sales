...
-------------------------------------------------------------------------------------------------------------------------
select count (distinct(state)) as Total_regions
from brightmotors.carsales.sales;
...
--Extracting Year, Month, Day, Hour, Minute, Second
select
      cast (to_timestamp(saledate, 'E MMM dd yyyy HH:mm:ss')) as Sales_timeline
from brightmotors.carsales.sales;
...
--Sales by different months grouped by sellers
select
      seller, year,
    case
        when split(saledate, ' ')[1] = 'Jan' then 'January'
        when split(saledate, ' ')[1] = 'Feb' then 'February'
        when split(saledate, ' ')[1] = 'Mar' then 'March'
        when split(saledate, ' ')[1] = 'Apr' then 'April'
        when split(saledate, ' ')[1] = 'May' then 'May'
        when split(saledate, ' ')[1] = 'Jun' then 'June'
        when split(saledate, ' ')[1] = 'Jul' then 'July'
        when split(saledate, ' ')[1] = 'Aug' then 'August'
        when split(saledate, ' ')[1] = 'Sep' then 'September'
        when split(saledate, ' ')[1] = 'Oct' then 'October'
        when split(saledate, ' ')[1] = 'Nov' then 'November'
        when split(saledate, ' ')[1] = 'Dec' then 'December'
    end as month_of_sale,
    count (*) as number_of_sales
from brightmotors.carsales.sales
group by all
order by number_of_sales desc;

--Sales by different months grouped by models
select
      model, year,
    case
        when split(saledate, ' ')[1] = 'Jan' then 'January'
...
--Top and bottom 10 models sold
select make,
       max (model) as top_10_models_sold,
       min(model) as bottom_10_models_sold
from brightmotors.carsales.sales
group by make
limit 10;
...
select
      ifnull(make, 'Not make selected') as make,
      ifnull(model, 'No model selected') as model,
      ifnull(color, 'No color selected') as color,
      model, year, seller, sellingprice, saledate, state,
    case
        when split(saledate, ' ')[1] = 'Jan' then 'January'
        when split(saledate, ' ')[1] = 'Feb' then 'February'
        when split(saledate, ' ')[1] = 'Mar' then 'March'
